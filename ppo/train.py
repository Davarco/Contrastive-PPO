    ...
    def train(self):
        ...
        for _ in range(self.n_epochs):
            for batch in self.rollout_buffer.get_dataloader(self.batch_size):
                ...
                critic_1 = torch.pow(c_values-returns, 2)
                critic_2 = torch.pow(values-returns, 2)
                critic_loss = 0.5*torch.mean(torch.max(critic_1, critic_2))
